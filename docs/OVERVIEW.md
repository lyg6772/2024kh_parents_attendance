...
- **하네스가 방금 들어왔고, 파이프라인은 아직 한 번도 돌지 않았다.** 첫 실행이 곧
  하네스 자체에 대한 첫 측정이다.
- **의존성 CVE 39건이 미해결이다** (13패키지, Critical 2 — `pymysql` 9.8,
  `h11` 9.1, `jinja2` 8.8, `pyjwt` 6건 중 하나는 상류 수정본 없음). 이 앱은 JWT 로
  인증하고 MySQL/Oracle 로 저장하므로 셋 다 실사용 경로 위다. **의존성 올리기가
  다음 작업 후보 1순위**이고, osv-scanner 가 pre-push 에서 이걸 막는다.
- **린트·타입 부채가 있다** (ruff 75건 · pyright 25건, 실측). 훅에는 비차단으로
